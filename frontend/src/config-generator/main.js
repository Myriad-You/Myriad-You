...
//
// 官网移植版：已移除 tapp 沙箱宿主（Tapp.api / Tapp.file / Tapp.ui / Tapp.i18n /
// 生命周期桥接），网络直连公共 API，通知走站内 Toast，界面语言跟随站点 locale，
// 由 ConfigGeneratorModal 挂载/卸载。
...
// 官网移植版：按站点 locale 选择词典，缺键回退到内置中文（I18N_FALLBACK）
var I18N_DICTS = { 'en-US': enUS, 'ja-JP': jaJP, 'zh-CN': zhCN };
...
function t(key, params) {
  var dict = I18N_DICTS[activeLocale] || I18N_FALLBACK;
  var raw = dict[key] || I18N_FALLBACK[key] || key;
...
async function resolveUpdaterDigest(tag) {
  ...
  if (typeof fetch === 'function') {
    // 官网优先走同源边缘代理（CF Pages Function，服务端可达 Docker Hub）；
    // 代理未部署/不可用时回退为浏览器直连
    try {
      var proxyResp = await fetch('/api/updater-digest?tag=' + encodeURIComponent(tag));
      if (proxyResp.ok) return extractDigestFromHubTag(await proxyResp.json());
    } catch (_) { /* 边缘代理不可用，回退直连 */ }
...
async function fetchDockerHubTags(repo) {
  ...
  if (typeof fetch === 'function') {
    try {
      var proxyResp = await fetch('/api/image-tags?repo=' + encodeURIComponent(repo));
      if (proxyResp.ok) return extractTagNamesFromHubPayload(await proxyResp.json());
    } catch (_) { /* 边缘代理不可用，回退直连 */ }
    var url = 'https://hub.docker.com/v2/repositories/somekawahitomi/' +
...
function downloadFile(content, filename) {
  // 沙箱 Tapp.file.download 通道已移除，直接走浏览器下载
...
/**
 * 挂载生成器：由 ConfigGeneratorModal 在 markup 注入后调用。
 * （原 tapp 沙箱 Tapp.lifecycle / window._TAPP_MODE 桥接已移除）
 * @param {string} [locale] 站点 locale（zh-CN / en-US / ja-JP），缺省沿用内置中文
 */
export function mountConfigGenerator(locale) {
  if (locale && I18N_DICTS[locale]) activeLocale = locale;
  initPage();
}

/** 卸载生成器：弹窗关闭时调用，解绑全部页面监听 */
export function unmountConfigGenerator() {
  disposePage();
}
